...
addFutureDayButton.addEventListener('click', (e) => {
  ...
  createDayObj(add(newestDay, {days: 1}), habitArray)
});
...
const idGen = () => {
  return 'id=' + Math.random().toString(36).substr(2, 8); //creates a string like "fz680z1"
};
...
function createDateHTMLTemplate(day) {
  let month = format(day, 'MMM'); //i.e. 'Jan'
  let dayOfMonth = format(day, 'd'); //i.e. 'Monday'
  let dayOfWeek = format(day, 'iiii'); //i.e. '18'
  let templateString =
